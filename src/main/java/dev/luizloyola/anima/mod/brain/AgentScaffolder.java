...
/**
 * The {@link Scaffolder} port over a live {@link AgentBody} — one nerd-pole step: jump, and while
 * airborne past block height, place a carried block into the cell just vacated. Ticked from
 * {@link AgentBody#serverAiStep()}, which owns the jump/place timing; the brain only asks for
 * steps.
 *
 * <p>Refusals are free: no such item, no headroom two above the feet, a step in flight, the ledger
 * at {@link Scaffolder#PILLAR_MAX} (un-build first), or a cell that has beaten them
 * {@link #ATTEMPTS_PER_CELL} times. A step that never clears block height within
 * {@link #STEP_TIMEOUT_TICKS} FAILS with nothing consumed.
 *
 * <p><b>The body centres in its cell before jumping.</b> The headroom check reads one column
 * ({@code feet.above(2)}) but the box is 0.6 wide, so off-centre it bonks a block that check never
 * looked at (decision: Luiz) — a leaf one cell over, the box 0.182 in, gave 95 dead jumps in a row.
 */
public final class AgentScaffolder implements Scaffolder {
    ...
    public AgentScaffolder(AgentBody person) {
        this.person = person;
    }
    ...
    /** One tick of climb work, from {@link AgentBody#serverAiStep()}; a no-op unless rising. */
    public void tick() {
        if (state != ScaffoldState.RISING) {
            return;
        }
        ...
        if (person.entity().getY() >= base.getY() + 1.0) {
            ...
            level.gameEvent(net.minecraft.world.level.gameevent.GameEvent.BLOCK_PLACE, base,
                    net.minecraft.world.level.gameevent.GameEvent.Context.of(person.entity(), blockState));
    ...
    private void fail(String detail) {
        ...
        if (failedStreak >= ATTEMPTS_PER_CELL) {
            // up() just returns false, and a silent give-up makes the next stuck AgentBody
